...
/** A bare (direct, non-ON-wrapped) TAP ability that's exactly one CHANGE(...) command whose gain side
 * includes BZ -- e.g. JOB004A's "CHANGE(3K,2BZ)". Returns the lowered CHANGE command, or null if faceId
 * has no TAP field, TAP isn't a single plain CHANGE, or that CHANGE doesn't produce BZ. See
 * MoveGenerator#forcedBzConversionMove's own doc for why this is treated as forced rather than a normal
 * candidate. Deliberately not scoped to JOB004A specifically -- any future card with the same shape
 * (a bare CHANGE-to-BZ ability) gets the same forced treatment automatically. */
function bzConversionTap(index, faceId) {
  let row;
  try { row = getCardRow(index, faceId); } catch (e) { return null; }
  if (!row.TAP) return null;
  const commands = lowerProgram(parse(row.TAP));
  if (commands.length !== 1 || commands[0].type !== 'CHANGE') return null;
  return commands[0].gain.some((g) => g.resource === 'BZ') ? commands[0] : null;
}
